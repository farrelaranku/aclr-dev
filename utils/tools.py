...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if args.lradj == 'type1':
        ...
        lr_adjust = {
            ...
            10: 1e-6, 15: 5e-7, 20: 1e-7
        }
    if epoch in lr_adjust.keys():
        lr = lr_adjust[epoch]
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr 
        print('No ACLR : Updating learning rate to {}'.format(lr))

def adjust_aclr(optimizer, loss, iter, var):
    current_loss = loss
    cycle = iter
    offset = cycle % 2
    ...
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
    # print('Using ACLR : Updating learning rate to {}'.format(lr))
